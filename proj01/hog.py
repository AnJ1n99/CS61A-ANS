...
def roll_dice(current_score, num_rolls, dice=six_sided):
    ...
    for _ in range(num_rolls):
        curr = dice()
        total += curr
        if current_score % 2 == curr % 2:  # is same parity
            extra += 1
        if curr == 1:
            pig_out = True
    return (1 if pig_out else total) + extra  # 3目
    # END PROBLEM 1
...
def swine_swap(player_score, opponent_score):
    ...
    """
    # BEGIN PROBLEM 4
    total = str(pow(3, player_score + opponent_score))
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    # BEGIN PROBLEM 7
    def say(score0, score1):
        currScore = score0 if who == 0 else score1
        gain = currScore - last_score
        if gain > running_high:
            print("{} point(s)! That's a record gain for Player {}!".format(gain, who))
        return announce_highest(who, currScore, max(running_high, gain))
    return say
...
def make_averaged(original_function, trials_count=1000):
    ...
    def trials(*args):
        ...
        for _ in range(trials_count):
            currTotal += original_function(*args)
        return currTotal / trials_count  #返回平均值
    return trials
    # END PROBLEM 8
...
def max_scoring_num_rolls(current_score=0, dice=six_sided, trials_count=1000):
    ...
    for num_rolls in range(1, 11): # 1 - 10 rolls
...
def run_experiments():
    ...
    print('hog_pile_strategy win rate:', average_win_rate(swine_swap_strategy))
