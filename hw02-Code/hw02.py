...
def protected_secret(password, secret, num_attempts):
    ...
    def get_secret(password_attempt):
        attempts = num_attempts     # 记录次数，递归模型
        if attempts <= 0:
            print("SECRET LOCKED")
        elif password_attempt == password:
            print(secret)                   # base case
        else:
            print("INCORRECT PASSWORD")
            attempts = attempts - 1
        return protected_secret(password, secret, attempts) # 自我调用
    return get_secret   
...
def one(f):
    ...
    return lambda x: f(x)   # successor(zero) == f(x)

def two(f):
    """Church numeral 2: same as successor(successor(zero))"""
    return lambda x: f(f(x))

three = successor(two)   # fffx

def church_to_int(n):
    """Convert the Church numeral n to a Python integer.

    >>> church_to_int(zero) # zero 不论传入是什么函数，均不理睬
    0
    >>> church_to_int(one)
    1
    >>> church_to_int(two)
    ...
    3
    """
    return n(increment)(0)   #n is a func  
