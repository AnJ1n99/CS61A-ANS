...
def pingpong(n):
    ...
    True
    """
    def helper(curr, ans, direct):
        if curr == n:  # base case
            return ans
        if curr % 6 == 0 or number_of_six(curr) > 0: # 包含6或者是6的倍数
            return helper(curr + 1, ans - direct, -direct)
        return helper(curr + 1, ans + direct, direct)
    return helper(1, 1, 1)
...
def missing_digits(n):
    ...
    if n < 10:
        ...
        flag = n % 10
        flag2 = n // 10 % 10
        if flag - flag2 > 1:  # expect flag1 == flag2
            return flag - flag2 - 1 + missing_digits(n // 10)
...
def count_change(total, next_money):
    ...
    True
    """
    def helper(remainder, min_money):
        if remainder == 0:
            return 1    # base case 1
        elif min_money is None or remainder < min_money:
            return 0    # base case 2
        else:
            return helper(remainder - min_money, min_money) + helper(remainder, next_money(min_money))
...
def move_stack(n, start, end):
    ...
    if n > 1:
        ...
        print_move(start, end)  # 不论有几个，均可将其看作是两组，一组是topdisk，一组是其余所有
