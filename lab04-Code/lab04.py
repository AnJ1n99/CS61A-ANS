...
def total_weight(m):
    ...
    if is_planet(m):
        return size(m)  # base case
    else:
        assert is_mobile(m), "le or a planet"
...
def balanced(m):
    ...
    False
    """
    # condition 1: left torque != right torque
    assert is_mobile(m)
    def torque(arm):
    ...
    # condition 2: The endpoints on both sides must either be planets or balanced mobiles
    def sub_banlanced(end):
        return is_planet(end) or balanced(end)
    left_end, right_end = end(left_arm), end(right_arm)
    return sub_banlanced(left_end) and sub_banlanced(right_end)
...
def preorder(t):
    ...
    [2, 4, 6]
    """
    result_list = [label(t)] #root[label]
    for b in branches(t):
